...
const CodingChallengesTab = () => {
    ...
    const [generateDifficulty, setGenerateDifficulty] = useState('EASY');
    ...
    const handleGenerateLeetCode = async (difficulty = 'EASY') => {
        setIsGenerating(true);
        try {
            // 1. Try MCP Challenge Server first (real-time LeetCode scraper + 10 test cases)
            const res = await fetch(`http://localhost:8008/mcp/create-weekly-challenge?difficulty=${difficulty}`, {
            ...
            if (res.ok) {
                ...
                alert(`✅ Successfully generated and scheduled LeetCode challenge: "${data.challenge.title}"!`);
                setIsGenerating(false);
                return;
            }
            throw new Error("MCP Server returned non-200");
        } catch (err) {
            console.warn("MCP Server not reachable, using client-side dynamic problem generator:", err.message);

            // 2. Client-side dynamic problem pool (10 test cases each, IST scheduled)
            const now = new Date();
            ...
            let satDate = new Date(istNow);
            if (dayOfWeek === 6) satDate.setHours(0, 0, 0, 0);
            else if (dayOfWeek === 0) { satDate.setDate(istNow.getDate() - 1); satDate.setHours(0, 0, 0, 0); }
            else { satDate.setDate(istNow.getDate() + (6 - dayOfWeek)); satDate.setHours(0, 0, 0, 0); }

            const pad = (n) => String(n).padStart(2, '0');
            const satIso = `${satDate.getFullYear()}-${pad(satDate.getMonth() + 1)}-${pad(satDate.getDate())}T00:00:00+05:30`;
            const sunIso = `${satDate.getFullYear()}-${pad(satDate.getMonth() + 1)}-${pad(satDate.getDate() + 1)}T23:59:59+05:30`;
            ...
            const fallbackPool = [
                {
                    ...
                    testCasesCount: 10,
                    testCases: [
                        { id: 1, input: '1 3 5 6\n5', output: '2', isHidden: false, description: 'Target exists in middle' },
                        { id: 2, input: '1 3 5 6\n2', output: '1', isHidden: false, description: 'Target between elements' },
                        { id: 3, input: '1 3 5 6\n7', output: '4', isHidden: true, description: 'Target greater than max' },
                        { id: 4, input: '1 3 5 6\n0', output: '0', isHidden: true, description: 'Target smaller than min' },
                        { id: 5, input: '1\n0', output: '0', isHidden: true, description: 'Single element smaller' },
                        { id: 6, input: '1\n1', output: '0', isHidden: true, description: 'Single element exact match' },
                        { id: 7, input: '1\n2', output: '1', isHidden: true, description: 'Single element larger' },
                        { id: 8, input: '1 2 4 6 8 10\n5', output: '3', isHidden: true, description: 'Even size array' },
                        { id: 9, input: '2 4 6 8 10\n6', output: '2', isHidden: true, description: 'Exact middle match' },
                        { id: 10, input: '10 20 30 40\n25', output: '2', isHidden: true, description: 'Two digits array' }
                    ]
                },
                {
                    ...
                    testCasesCount: 10,
                    testCases: [
                        { id: 1, input: 'A man, a plan, a canal: Panama', output: 'true', isHidden: false, description: 'Classic palindrome phrase' },
                        { id: 2, input: 'race a car', output: 'false', isHidden: false, description: 'Non-palindrome string' },
                        { id: 3, input: ' ', output: 'true', isHidden: true, description: 'Empty string after strip' },
                        { id: 4, input: '0P', output: 'false', isHidden: true, description: 'Digit and letter mismatch' },
                        { id: 5, input: 'a.', output: 'true', isHidden: true, description: 'Single char with punctuation' },
                        { id: 6, input: 'ab_a', output: 'true', isHidden: true, description: 'Underscore handling' },
                        { id: 7, input: 'Madam, In Eden, I\'m Adam', output: 'true', isHidden: true, description: 'Mixed case with punctuation' },
                        { id: 8, input: '12321', output: 'true', isHidden: true, description: 'Numeric palindrome' },
                        { id: 9, input: '123456', output: 'false', isHidden: true, description: 'Numeric non-palindrome' },
                        { id: 10, input: 'No lemon, no melon', output: 'true', isHidden: true, description: 'Complex sentence' }
                    ]
                },
                {
                    ...
                    testCasesCount: 10,
                    testCases: [
                        { id: 1, input: '1 2 3', output: '1 2 4', isHidden: false, description: 'Simple increment' },
                        { id: 2, input: '4 3 2 1', output: '4 3 2 2', isHidden: false, description: 'Multiple digits' },
                        { id: 3, input: '9', output: '1 0', isHidden: true, description: 'Single 9 overflow' },
                        { id: 4, input: '9 9 9', output: '1 0 0 0', isHidden: true, description: 'All 9s overflow' },
                        { id: 5, input: '1 9 9', output: '2 0 0', isHidden: true, description: 'Middle 9 cascade' },
                        { id: 6, input: '0', output: '1', isHidden: true, description: 'Zero' },
                        { id: 7, input: '8 9 9 9', output: '9 0 0 0', isHidden: true, description: 'Trailing 9s' },
                        { id: 8, input: '7 2 8 5 0 9 1 2 9 5 3 6 6 7 3 2 8 4 3 7 9 5 7 7 4 7 4 9 4 7 0 1 1 1 7 4 0 0 6', output: '7 2 8 5 0 9 1 2 9 5 3 6 6 7 3 2 8 4 3 7 9 5 7 7 4 7 4 9 4 7 0 1 1 1 7 4 0 0 7', isHidden: true, description: 'Very large array' },
                        { id: 9, input: '1 0 0 0', output: '1 0 0 1', isHidden: true, description: 'Zero ending' },
                        { id: 10, input: '5 9', output: '6 0', isHidden: true, description: 'Two digits with 9' }
                    ]
                },
                {
            ...
            alert(`✅ Random LeetCode challenge scheduled: "${chosen.title}"!`);
        } finally {
            setIsGenerating(false);
        }
    };
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
            <div className="flex justify-between items-end">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    >
                        <option value="EASY">Easy (200 XP)</option>
                        <option value="MEDIUM">Medium (500 XP)</option>
                    </select>

                    <button
                        onClick={() => handleGenerateLeetCode(generateDifficulty)}
                        disabled={isGenerating}
                        className="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white font-bold rounded-xl flex items-center gap-2 shadow-lg transition-all text-xs"
                    >
                        <Code2 className={`w-4 h-4 ${isGenerating ? 'animate-spin' : ''}`} />
                        {isGenerating ? 'Fetching LeetCode Problem...' : 'Generate LeetCode Challenge (10 Test Cases)'}
                    </button>
                    ...
                    <button
                        ...
                        className="px-4 py-2.5 bg-blue-600 text-white font-bold rounded-xl flex items-center gap-2 shadow-lg shadow-blue-200 hover:bg-blue-700 transition-all text-xs"
                    >
