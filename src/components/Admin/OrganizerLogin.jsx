...
const OrganizerLogin = () => {
    ...
    return (
        <div className="h-[100dvh] w-full flex flex-col md:flex-row md:items-center justify-center bg-[#bfdbfe] overflow-hidden font-sans relative">
            ...
            {/* Page Background Abstract Waves to match target image but in Blue */}
            ...
            <div className="flex md:hidden flex-col items-center text-center pt-10 pb-6 px-6 relative z-10 w-full">
                ...
                <p className="text-[13.5px] text-gray-700/80 font-medium max-w-[280px] leading-relaxed">
                    Please enter your organizer credentials to access the event workspace.
                </p>
            </div>
            ...
            >
                {/* DESKTOP ONLY: Left Panel - Blue Gradient with Abstract Elements */}
                <div className="hidden md:flex flex-col justify-center w-[48%] p-12 lg:p-14 relative overflow-hidden bg-[#2563eb] text-white">
                    ...
                    <div className="relative z-10 -mt-10">
                        <h2 className="text-[2.5rem] lg:text-[2.75rem] font-bold mb-4 leading-tight tracking-tight">Welcome Organizer!</h2>
                        <p className="text-[1rem] lg:text-[1.05rem] text-white/90 max-w-[290px] leading-relaxed">
                            Securely authenticate to access the event management workspace and organize events.
                        </p>
                    </div>
                </div>

                {/* Right Panel - White Login Form */}
                <div className="w-full md:w-[52%] px-6 sm:px-10 py-8 md:p-10 lg:p-10 flex flex-col bg-[#fdfdfd] md:bg-white relative z-10 h-full overflow-y-auto custom-scrollbar">
                    ...
                    <form onSubmit={handleLogin} className="space-y-4">
                        ...
                        <div className="flex items-center justify-between px-2 pt-2 md:pt-1">
                            ...
                            <button type="button" onClick={() => setError('Please contact the System Administrator to reset your organizer credentials.')} className="text-[13px] text-gray-500 font-medium hover:text-[#243b55] md:hover:text-[#2563eb] transition-colors">
                                Forgot Password?
                            </button>
                        </div>

                        <div className="mt-4 p-3 bg-blue-50/80 border border-blue-100 rounded-xl flex items-start gap-3">
                            <ShieldAlert className="w-[18px] h-[18px] text-blue-500 shrink-0 mt-0.5" />
                            <p className="text-[11px] text-blue-700 leading-tight">
                                <strong className="font-semibold block mb-0.5">Authorized Organizers Only</strong>
                                This portal is restricted. Access is logged and monitored.
                            </p>
                        </div>
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full bg-[#243b55] hover:bg-[#1a2a3d] md:bg-[#1d4ed8] md:hover:bg-[#1e40af] text-white font-medium py-[16px] md:py-[14px] rounded-2xl md:rounded-full shadow-lg md:shadow-[0_8px_20px_rgba(29,78,216,0.25)] md:hover:shadow-[0_12px_25px_rgba(29,78,216,0.35)] transition-all duration-300 mt-4 mb-2 md:mb-0 flex items-center justify-center disabled:opacity-70 text-[15px]"
                        >
                            {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <span className="md:hidden">Log Me In</span>}{!loading && <span className="hidden md:inline">Enter Dashboard</span>}
                        </button>
                    </form>

                    <div className="mt-auto pt-6 md:pt-0 md:mt-6 text-center text-[12px] font-medium text-gray-500 pb-4 md:pb-0">
                        Switch to <span onClick={() => navigate('/admin/login')} className="text-[#243b55] md:text-[#2563eb] cursor-pointer hover:underline ml-1 font-semibold">Core Admin</span>
                    </div>
                </div>
            </motion.div>
        </div>
